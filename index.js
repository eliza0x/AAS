...
(async function(){
    ...
    const votes = await page.evaluate(() => {
      ...
      return labos.map(line => {
        ...
        /*
        const labo_name  = line_arr[0].innerHTML;
        const labo_limit = line_arr[2].innerHTML;
        const labo_cnt   = line_arr[3].innerHTML;
        return {name: labo_name, cnt: labo_cnt, limit: labo_limit};
        */
        return line_arr.map(line=>line.innerHTML)
      });
    });
    // votes.map(x => console.log(x.name+': '+x.cnt+'/'+x.limit))
    votes.map(x => console.log(x+'\n'))
